...
switch(d)
    case 1
        for i = 1:size(tvb_prt,2)
            line([tvb_prt{i}(1) tvb_prt{i}(2)],[0 0],'Color',col(i,:));
            hold on;
            plot(tvb_prt{i}(:,1),0,'Marker','o','MarkerSize',6,'MarkerEdgeColor',col(i,:))
        end
        plot(tvb_pnt(1),0,'Marker','o','MarkerSize',12,'MarkerFaceColor','r', 'MarkerEdgeColor','b')
        axis('equal')
        axis([0 1 -0.1 0.1]);
        set(gca,'FontSize',16);
%         set(gca,'XtickLabel',[]);
    ...
    case 2
        for i = 1:size(tvb_prt,2)
            if size(tvb_prt{i},1) >= d+1
                K = convhull(tvb_prt{i});
                line(tvb_prt{i}(K,1),tvb_prt{i}(K,2),'Color',col(i,:));
            else
                line(tvb_prt{i}(:,1),tvb_prt{i}(:,2),'Color',col(i,:));
            end
            hold on;
        end
        plot(tvb_pnt(1),tvb_pnt(2),'Marker','o','MarkerSize',12,'MarkerFaceColor','r', 'MarkerEdgeColor','b')
        axis('equal')
        axis([0 1 0 1]);
        set(gca,'xtick',[0 1]);
        set(gca,'ytick',[0 1]);
